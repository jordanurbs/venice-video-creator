...
- [ ] Make placement, reorder, replacement, and keep/duck/mute operate on linked clip groups. Restore audio when returning to keep; preserve manual edits and one-operation undo.
  ...
  - [x] Shared native/tool plan mutations apply keep/duck/mute to the bound linked pair, retain base mix levels across retakes/reopen, and restore legacy missing native sound where the source/binding is unambiguous. Missing/detached bindings fail atomically; authored fades and dB curves survive. One-step undo/redo is covered; native UI acceptance and general timeline reconciliation remain open.
- [ ] Persist stable operation and line identities before asynchronous waits, including placeholders, remote queue IDs, recipes, destination bindings, stage, and retry state.
  ...
  - [x] Audio attempts retain line/bed-role identity, recipe, destination clip, timing/mix ownership, placeholder, backend/queue metadata, and measured output facts. Shared service checkpoints these before provider access; tool edits preserve unchanged line IDs and accept explicit IDs for revisions. Legacy named production audio blocks automatic duplication; adopting arbitrary old clips remains open.
...
- [ ] Make audio reruns idempotent per durable line/role identity; partial edits replace intended audio without duplicates.
  - [x] New-format attempts reuse identical requests and replace exact clip IDs for changed recipes or explicit regeneration. Reversed completion, simultaneous replacements, cancellation, saved-package recovery, and failed final-save retries are covered. Removed/changed bindings fail visibly; legacy adoption and native acceptance remain open.
- [ ] Size dialogue from decoded output and recompute timing/ducking after generation, trim, move, retake, and reopen.
  - [x] Shared live/recovered finishing reads AVFoundation audio-track/duration facts, refreshes asset/manifest duration, reflows automatic dialogue within picture, and recomputes owned bed envelopes. Explicit retries can accept intervening manual edits without another generation. Continuous reconciliation after arbitrary timeline edits is next.
  - [x] Shared `reconcile_audio` / native Reconcile audio action refits already-placed output after edits, rebases FPS ownership, preserves manual spans/envelopes, and restores both layout and metadata through undo/redo. It requires settled, present, current audio and rejects conflicts atomically. Automatic lifecycle hooks and export readiness enforcement remain open.
- [ ] Fit music/ambient beds to the intended cut rather than full provider duration; retain intentional fades and manual edits.
  - [x] Beds use picture extent, trim excess source duration, reject short coverage, and preserve edited fades/mix/envelopes. Finite-duration models must offer a duration long enough for the cut. Later cut changes and intentional short/manual beds still need the general timing reconciliation path.
  - [x] Corrected linear-gain values being written into decibel keyframe tracks. Playback-volume regressions now prove attenuation and preserve the independent static mix gain.
  - [x] Merge overlapping duck ramps across nearby lines and keep speech at the cut ducked instead of interpolating toward full volume throughout the window.
- [ ] Handle speech overruns explicitly against picture timing; never silently push dialogue into an unrelated shot.
  - [x] New-format finishing retains overlong output with an actionable blocked state instead of spilling into the next shot. Extending picture permits finishing the same output without regeneration.
- [ ] Separate native speech, voiceover, and exact on-screen speech. Ensure each line is audible once; do not blindly add TTS over native dialogue.
  - [x] `produce_audio` generates only `voiceOver` lines; on-screen lines are reported as `nativeSpeechUnverified`. Native speech ducking uses picture windows, not claimed measured speech. Transcript/lip-sync verification and keep/duck/mute restoration remain open.
...
- Shared video finalization committed as `8d3e3e8` (`feat(production): finalize retained takes with per-beat review`), excluding the unrelated bitrate hunk.
- Durable audio slice: persisted per-line/bed-role attempts, pre-provider checkpoints, exact replacements, measured live/recovered finishing, cut-length beds, owned ducking, and explicit overrun/manual-edit conflicts. Added 19 audio regressions, including a real WAV/package-reopen fixture. Targeted 47 tests in four suites and full 1,137 tests in 173 suites pass (seven skips). Next: shared timeline-edit reconciliation and native audio policy restoration, then readiness/export; legacy adoption, native UI, real queue recovery, and exact speech remain unverified.
- Durable audio committed as `9c91687` (`fix(production): retain audio attempts and finish measured takes`), excluding the unrelated bitrate hunk.
- Native mix slice: persisted base/applied volume state per bound clip; shared atomic keep/duck/mute, linked-source restoration, and undo; mix-only changes excluded from storyboard/video destination identity. Corrected bed ducking from linear gain values to dB envelopes and asserted actual playback gain. Eight new mix regressions plus the updated measured-audio test pass: targeted 60 tests in five suites; full 1,145 tests in 174 suites (seven skips). Next: shared reconciliation after arbitrary timeline edits and retained export/readiness.
- Native mix/ducking committed as `b89fcf1` (`fix(audio): restore native mix and apply decibel ducking`), excluding the unrelated bitrate hunk.
