...
- [x] **5.3 Wan audio preflight/padding — DONE (needs live confirm).** Enabled `maxReferenceAudios: 1` for audio_url-capable families (`VideoModelCapabilities.audioInputCapable`: Wan 2.5/2.6/2.7 except 2.7 R2V, DaVinci MagiHuman) so `referenceAudioMediaRefs`→`audio_url` finally fires. Added `AudioSilencePadder.padIfShorter` — native AVFoundation trailing-silence pad (AVMutableComposition + `insertEmptyTimeRange` → m4a export, no ffmpeg, source never mutated) — wired into `VideoGenerationSubmission`'s existing `preprocessRef` hook, gated on `VideoModelCapabilities.minAudioInputSeconds` (Wan 2.7 / MagiHuman = 3s). Corrected the misleading `referenceAudioMediaRefs` tool description. Build green; capability gating unit-tested (886 tests). ⚠️ **Live-verify:** the padder's end-to-end test destabilized the parallel suite (AVAssetExportSession under cross-suite concurrency) so it was removed after passing in isolation — run a real Wan-2.7 lip-sync gen with a <3s clip to confirm the padded audio is accepted and the audio-ref UI slot behaves.
...
- 2026-07-06 — Phase 5.3 done: enabled audio_url (`maxReferenceAudios`) for Wan/MagiHuman via VideoModelCapabilities, plus native AVFoundation trailing-silence padding (AudioSilencePadder) wired into the submission preprocessRef, gated on the 3s Wan/MagiHuman floor. Fixed the referenceAudioMediaRefs tool description. Capability gating unit-tested; the export-based padder test was dropped (destabilized the parallel suite; passed in isolation). Needs a live Wan lip-sync run to confirm end-to-end. 886 tests green.
