...
- Extract `EditorViewModel.importFinderItemsForPlacement(_:) async -> [MediaAsset]` (duplicated in `Timeline/TimelineView.swift` + `Preview/PreviewContainerView.swift`); make the per-asset `loadMetadata` loop a task group (20-file drops currently serialize).
...
- `VeniceJobStore.activeCount` → `tasks.count`; retire the `mediaPanelToast` forwarding alias (rename call sites to `editorToast`).
...
- Perf: lazy `deletionImpactCount` in the folder tile contextMenu (`MediaTab+Grids.swift`, currently O(F×(F+A)) per render); stop calling `rebuildToolTips()` from `draw()` (`TimelineHeaderView.swift` — rebuild on track mutation); restore concurrent `async let` in `AccountService.refreshUsage`; replace `waitWhileExportActive`'s 2s poll with continuations resumed in `endExport()`.
