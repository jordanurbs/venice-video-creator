        ...
        AgentTool(
            ...
            inputSchema: objectSchema(
                properties: [
                    ...
                    "slots": [
                        "type": "array",
                        "description": "One entry per slot of the chosen layout. Each entry names a 'slot' and gives exactly one of 'mediaRef' (place a new clip), 'clipId' (re-layout one existing clip), or 'clipIds' (re-layout a batch of existing clips into the same slot). Don't mix placement (mediaRef) with re-layout (clipId/clipIds) across slots.",
                        "items": objectSchema(
                            properties: [
                                "slot": ["type": "string", "description": "Slot name for the chosen layout (e.g. 'left', 'inset', 'top_right')."],
                                "mediaRef": ["type": "string", "description": "Asset ID from get_media to place into this slot. Use this OR clipId/clipIds."],
                                "clipId": ["type": "string", "description": "Existing clip to move into this slot (transform/crop only). Use this OR mediaRef/clipIds."],
                                "clipIds": [
                                    "type": "array",
                                    "items": ["type": "string"],
                                    "description": "Batch of existing clips to move into this slot — every listed clip gets this slot's transform/crop (e.g. a track of sequential takes all framed into 'left'). Use this OR mediaRef/clipId. Clips sharing a slot may sit on the same track; clips in DIFFERENT slots still must not overlap on one track.",
                                ],
                                "anchor": [
                                    "type": "string",
