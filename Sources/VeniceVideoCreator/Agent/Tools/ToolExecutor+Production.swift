...
extension ToolExecutor {
    ...
    func produceShots(_ editor: EditorViewModel, _ args: [String: Any]) throws -> ToolResult {
        ...
        guard !editor.productionOrchestrator.isRunning else {
            throw ToolError("A production run is already in progress. Use production_status to check it, or wait for it to finish.")
        }
        ...
        let body: [String: Any] = [
            ...
            "hint": "Production runs in the background. Progress posts into chat; poll get_shot_plan for per-shot status (generating → placed/failed) or production_status for run counters.",
        ]
    ...
    /// Optionally overrides the prompt or model first. Runs through the orchestrator, so it
    /// won't start while another run is active.
    func regenerateShot(_ editor: EditorViewModel, _ args: [String: Any]) throws -> ToolResult {
        ...
        guard !editor.productionOrchestrator.isRunning else {
            throw ToolError("A production run is already in progress. Wait for it to finish, then regenerate.")
        }
        ...
        let body: [String: Any] = [
            "started": true,
            "shotId": shotId,
            "hint": "New take generating. It replaces the shot's timeline clip in place when done; earlier takes are kept in the shot's take history. Poll get_shot_plan.",
        ]
        return .ok(Self.jsonString(body) ?? "{}")
    }
