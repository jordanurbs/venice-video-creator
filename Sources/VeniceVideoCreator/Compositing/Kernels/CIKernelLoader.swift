...
enum CIKernelLoader {
    private static func metallibURL(_ lib: String) -> URL? {
        ...
        let candidates = [
            ...
            Bundle.main.resourceURL?.appendingPathComponent("VeniceVideoCreator_VeniceVideoCreator.bundle/\(lib).metallib"), // swift run
            buildDir.appendingPathComponent("VeniceVideoCreator_VeniceVideoCreator.bundle/\(lib).metallib"),                 // swift test
        ].compactMap { $0 }
