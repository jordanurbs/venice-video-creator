...
struct GenerationInput: Codable, Sendable, Equatable {
    ...
    var productionAudioOperationId: String?
    ...
    /// Video/image seed for reproducible generation. Only emitted to families
    /// that accept it (`VideoModelCapabilities.supportsSeed`); recorded on the
    /// take recipe so a specific take can be replayed (harness rule 39/seeds).
    var seed: Int?
