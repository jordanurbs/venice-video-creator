...
if [ -f "$RES_BUNDLE/venice-video-creator.mcpb" ]; then
  cp "$RES_BUNDLE/venice-video-creator.mcpb" "$APP/Contents/Resources/"
else
  echo "!! missing venice-video-creator.mcpb in SwiftPM resource bundle at $RES_BUNDLE" >&2
  exit 1
fi
...
if [ -d "$RES_BUNDLE/Capabilities" ]; then
  cp -R "$RES_BUNDLE/Capabilities" "$APP/Contents/Resources/"
else
  echo "!! missing Capabilities/ in SwiftPM resource bundle at $RES_BUNDLE — bundled capability manifest would be missing" >&2
  exit 1
fi
